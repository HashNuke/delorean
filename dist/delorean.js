...
(function() {
  this.Pilgrim = (function() {
    var numberOfDaysInMonth, padZero;

    Pilgrim.prototype.value = {};

    function Pilgrim($input, options) {
      this.$input = $input;
      ...
      this.view = new Pilgrim.View(this, this.options.startingView);
    }

    Pilgrim.prototype.setDefaultDateIfNecessary = function() {
      var _base, _base1;
    ...
    Pilgrim.prototype.setDefaultOptions = function() {
    ...
    Pilgrim.prototype.years = function(yearAmongRange) {
    ...
    Pilgrim.prototype.months = function(year) {
    ...
    Pilgrim.prototype.shortWeekdayName = function(weekdayId) {
      if (this.lang.days[weekdayId] != null) {
        return this.lang.days[weekdayId].short;
      }
    };

    Pilgrim.prototype.getLocale = function(languageCode) {
      if (Pilgrim.Locale[languageCode] != null) {
        return Pilgrim.Locale[languageCode];
      }
      throw new Pilgrim.Error("Locale not available");
    };

    Pilgrim.prototype.days = function(year, month) {
    ...
    Pilgrim.prototype.setValue = function(year, month, day) {
    ...
    Pilgrim.prototype.format = function() {
      switch (this.options.format) {
        ...
        default:
          return new Pilgrim.Error("Invalid format string");
      }
    };

    Pilgrim.prototype.parseInitialValue = function() {
      var dateParts;
      ...
      switch (this.options.format) {
        ...
        default:
          return new Pilgrim.Error("Initial value is of unknown format");
      }
    };
  ...
  this.Pilgrim.Error = (function() {
    Error.prototype.name = "Pilgrim.Error";
  ...
  this.Pilgrim.View = (function() {
    function View(pilgrim, startingView) {
      var days;
      this.pilgrim = pilgrim;
      this.layout();
      if (startingView === "days") {
        days = this.pilgrim.days(this.pilgrim.value.year, this.pilgrim.value.month);
        this.daysView(this.pilgrim.value.year, this.pilgrim.value.month, days);
      } else if (startingView === "months") {
        this.monthsView(this.pilgrim.value.year, this.pilgrim.months(this.pilgrim.value.year));
      } else {
        this.yearsView(this.pilgrim.years());
      }
    }
    ...
    View.prototype.layout = function() {
      this.$root = $("<div/>").addClass("pilgrim");
      this.$header = $("<div/>").addClass("pilgrim-header");
      this.$content = $("<div/>").addClass("pilgrim-content");
    ...
    View.prototype.bindEvents = function() {
      this.$root.on("click", ".year", (function(_this) {
        return function(event) {
          ...
          return _this.monthsView(year, _this.pilgrim.months(year));
        };
      })(this));
      this.$root.on("click", ".year-nav", (function(_this) {
        return function(event) {
          var year;
          year = $(event.target).data("year");
          return _this.yearsView(_this.pilgrim.years(year));
        };
      })(this));
      this.$root.on("click", ".month", (function(_this) {
        return function(event) {
          ...
          return _this.daysView(year, month, _this.pilgrim.days(year, month));
        };
      })(this));
      this.$root.on("click", ".valid-day", (function(_this) {
        return function(event) {
          ...
          _this.pilgrim.setValue(year, month, day);
          _this.pilgrim.$input.val(_this.pilgrim.format());
          return $(document).trigger("pilgrim:destroy");
        };
      })(this));
    ...
    View.prototype.yearsView = function(years) {
      ...
      for (index = _i = 0, _len = years.length; _i < _len; index = ++_i) {
        ...
        if ([0, 3, 7].indexOf(index) !== -1) {
          this.$content.append($("<div/>").addClass("pilgrim-row"));
        }
    ...
    View.prototype.daysView = function(year, month, days) {
      ...
      this.$header.append(this.yearHeaderNav(year));
      this.$content.append($("<div/>").addClass("pilgrim-row").addClass("pilgrim-weekdays"));
      _ref = days.slice(0, 7);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        dayInfo = _ref[_i];
        weekdayName = this.pilgrim.shortWeekdayName(dayInfo.weekdayId);
    ...
    View.prototype.monthHeaderNav = function(year, month) {
      return $("<span/>").addClass("change-month").data({
        year: year
      }).text(this.pilgrim.lang.months[month].long);
    };
  ...
  $.fn.datepicker = function(options) {
    ...
    this.on("focusin", function(event) {
      ...
      return $("body").addClass("pilgrim-open").data("pilgrim", pilgrim);
    });
    $(window).on("datepicker:destroy", function() {
      var pilgrim;
      pilgrim = $("body").data("pilgrim");
      pilgrim.destroy();
      return $("body").removeData("pilgrim").removeClass("pilgrim-open");
    });
    return $(window).on("click", function(event) {
      var $target, isChildOfPilgrimElement, isElementInDom, isPilgrimElement, isPilgrimInput, isPilgrimOpen;
      $target = $(event.target);
